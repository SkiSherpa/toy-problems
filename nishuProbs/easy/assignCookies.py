...
# matching or greater than, number in lists g and s
# si, gi, count = 0
# sort each list
# loop through g
    # if the current in s is <= to current in g
        # make count go up by one
        # delete current items in s and g
        # minus one to si, gi
    # add one to indexes
...
class Solution:
    def findContentChildren(self, g: list[int], s: list[int]) -> int:
        ...
        g.sort()
        s.sort()
        while len(s) > 0 and len(g) > 0 and gi < len(g) and si < len(s):
            if g[gi] <= s[si]:
                count += 1
                si += 1
                gi += 1
